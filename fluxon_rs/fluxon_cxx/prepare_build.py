...
YAML_PATH = SCRIPT_DIR / "prepare_build.yaml"
...
def parse_prepare_targets_line(
    ...
    if current_name is None:
        raise RuntimeError("prepare_build.py: PREPARE_TARGETS entry is missing its target name header")
    ...
    raise RuntimeError(f"prepare_build.py: unsupported PREPARE_TARGETS line: {stripped}")
...
def parse_prepare_scenarios_line(
    ...
    if current_name is None:
        raise RuntimeError("prepare_build.py: PREPARE_SCENARIOS entry is missing its scenario name header")
    ...
    if indent == 6 and current_list_name == "targets" and stripped.startswith("- "):
        ...
        return current_name, current_list_name
    raise RuntimeError(f"prepare_build.py: unsupported PREPARE_SCENARIOS line: {stripped}")
...
def parse_cache_steps_line(
    ...
    if current_name is None:
        raise RuntimeError("prepare_build.py: CACHE_STEPS entry is missing its step name header")
    ...
    if indent == 6 and current_list_name in ("scenarios", "inputs", "outputs") and stripped.startswith("- "):
        ...
        return current_name, current_list_name
    raise RuntimeError(f"prepare_build.py: unsupported CACHE_STEPS line: {stripped}")
...
def validate_prepare_config(cfg: dict) -> None:
    ...
    if not targets:
        raise RuntimeError("prepare_build.py: PREPARE_TARGETS is empty")
    if not scenarios:
        raise RuntimeError("prepare_build.py: PREPARE_SCENARIOS is empty")
    for target_name, target_cfg in targets.items():
        if not str(target_cfg.get("dir_name", "")).strip():
            raise RuntimeError(
                f"prepare_build.py: PREPARE_TARGETS.{target_name}.dir_name must be set"
            )
        target_kind = str(target_cfg.get("kind", "")).strip()
        if target_kind not in ("system_resource", "remote_archive"):
            raise RuntimeError(f"prepare_build.py: unsupported target kind for {target_name}: {target_kind}")
        if target_kind == "remote_archive":
            if not str(target_cfg.get("url", "")).strip():
                raise RuntimeError(f"prepare_build.py: PREPARE_TARGETS.{target_name}.url must be set")
            if not str(target_cfg.get("verify", "")).strip():
                raise RuntimeError(f"prepare_build.py: PREPARE_TARGETS.{target_name}.verify must be set")
            if str(target_cfg.get("verify_format", "")).strip() != "first_token_sha256":
                raise RuntimeError(
                    f"prepare_build.py: PREPARE_TARGETS.{target_name}.verify_format must be first_token_sha256"
                )
    for scenario_name, scenario_cfg in scenarios.items():
        targets_list = scenario_cfg.get("targets")
        if not isinstance(targets_list, list) or not targets_list:
            raise RuntimeError(
                f"prepare_build.py: PREPARE_SCENARIOS.{scenario_name}.targets must not be empty"
            )
        binary_target = scenario_cfg.get("binary_target")
        if binary_target not in targets:
            raise RuntimeError(
                f"prepare_build.py: PREPARE_SCENARIOS.{scenario_name}.binary_target must reference a target"
            )
        for target_name in targets_list:
            if target_name not in targets:
                raise RuntimeError(
                    f"prepare_build.py: PREPARE_SCENARIOS.{scenario_name} references unknown target {target_name}"
                )
    for step_name, step_cfg in cache_steps.items():
        for key in ("scenarios", "inputs", "outputs"):
            ...
            if not isinstance(values, list) or not values:
                raise RuntimeError(
                    f"prepare_build.py: CACHE_STEPS.{step_name}.{key} must be a non-empty list"
                )
...
def resolve_target(cfg: dict, target_name: str) -> dict:
    try:
        ...
        raise RuntimeError(f"prepare_build.py: unknown prepare target {target_name}") from err
...
def resolve_scenario(cfg: dict, scenario_name: str) -> dict:
    try:
        ...
        raise RuntimeError(f"prepare_build.py: unknown prepare scenario {scenario_name}") from err
...
def parse_verify_payload(verify_payload_text: str) -> str:
    ...
    if not first_token:
        raise RuntimeError("prepare_build.py: empty verify payload")
...
def populate_remote_archive_resource(target_cfg: dict, install_root: Path) -> None:
    ...
    if not archive_path.is_file():
        print(f"[prepare_build] downloading packed resource from {download_url}")
        download_file(download_url, archive_path)
    if not verify_path.is_file():
        print(f"[prepare_build] downloading packed resource verify file from {verify_url}")
    ...
    if expected_sha256.lower() != actual_sha256.lower():
        raise RuntimeError(
            "prepare_build.py: archive integrity verification failed for "
    ...
    with tempfile.TemporaryDirectory(prefix=f"{install_root.name}_extract_") as temp_dir_str:
        ...
        print(f"[prepare_build] extracting {archive_path} -> {install_root}")
...
def get_target_dir() -> Path:
    ...
    return (workspace_root / target_dir).resolve()
...
def copy_binary_from_path(binary_name: str, dst_dir: Path) -> None:
    ...
    if source_path is None:
        raise RuntimeError(f"prepare_build.py: required system binary is missing from PATH: {binary_name}")
...
def copy_vendor_runtime_headers(install_root: Path) -> None:
    ...
    if rdma_src is None:
        raise RuntimeError("prepare_build.py: missing system RDMA headers under /usr/include/rdma")
    if infiniband_src is None:
        raise RuntimeError(
            "prepare_build.py: missing system ibverbs headers under /usr/include/infiniband"
        )
...
def copy_vendor_runtime_driver_configs(install_root: Path) -> None:
    ...
    if driver_src is None:
        raise RuntimeError("prepare_build.py: missing system libibverbs driver configs under /etc/libibverbs.d")
...
def copy_vendor_runtime_libraries(install_root: Path) -> None:
    ...
    for prefix in REQUIRED_VENDOR_RUNTIME_LIB_PREFIXES:
        ...
        if not copied:
            raise RuntimeError(
                f"prepare_build.py: missing required system vendor runtime library with prefix {prefix}"
            )
    for prefix in ("librdmacm.so", "libefa.so", "libnl-3.so", "libnl-route-3.so"):
...
def copy_cxxpacked_projection(install_root: Path) -> None:
    ...
    for prefix in ("libibverbs.so", "libmlx5.so"):
        ...
        if not copied:
            raise RuntimeError(
                f"prepare_build.py: missing required system cxxpacked projection library with prefix {prefix}"
            )
    ...
    if not copied_provider:
        raise RuntimeError(
            "prepare_build.py: missing required system cxxpacked provider library with prefix libmlx5-rdmav"
        )
...
def system_resource_ready(resource_url: str, packed_dir: Path) -> bool:
    if resource_url != "system://vendor_runtime":
        raise RuntimeError(f"prepare_build.py: unsupported system resource {resource_url}")
...
def install_binaries(install_root: Path, binaries: list[str]) -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        print("[prepare_build] skip installing binaries to /usr/bin (not running as root)")
        return
...
def ensure_meson(target_dir: Path) -> None:
    ...
    print(f"[prepare_build] downloading Meson from {url}")
    subprocess.run(
        [
            "curl",
            "-L",
    ...
    print(f"[prepare_build] extracting {tar_path} -> {meson_root}")
    subprocess.run(
...
def materialize_prepare_target(target_cfg: dict, target_dir: Path) -> Path:
    ...
    if dir_name == "vendor_runtime":
        if target_kind != "system_resource":
            raise RuntimeError("prepare_build.py: vendor_runtime must be a system_resource target")
        ...
        if not system_resource_ready(str(target_cfg["url"]).strip(), install_root):
            raise RuntimeError(
                f"prepare_build.py: prepared system resource remained incomplete: {install_root}"
            )
        ...
        if target_kind == "remote_archive":
            ...
            raise RuntimeError(f"prepare_build.py: unsupported target kind for cxxpacked: {target_kind}")
        if not cxxpacked_projection_ready(install_root):
            raise RuntimeError(
                f"prepare_build.py: prepared cxxpacked projection remained incomplete: {install_root}"
            )
    else:
        raise RuntimeError(f"prepare_build.py: unsupported prepare target dir_name={dir_name}")
...
def resolve_binary_path(install_root: Path, binary_name: str) -> Path:
    ...
    if not binary_path.is_file():
        raise RuntimeError(
            f"prepare_build.py: requested binary is missing from prepared target: {binary_path}"
        )
