...
use fluxon_observability::keys::{
    ...
    PROM_METRIC_KV_PEER_NETWORK_BYTES_TOTAL, PROM_METRIC_SHM_FILE_ALLOCATED_BYTES,
    ...
    PROM_METRIC_MQ_PREFETCH_TARGET_INFLIGHT, PROM_METRIC_MQ_PUT_WINDOW_BYTES,
    PROM_METRIC_MQ_PUT_WINDOW_CALLS, PROM_METRIC_MQ_PRODUCER_NONBLOCKING_LATEST_INTERVAL_UNIX_MS,
    PROM_METRIC_MQ_PRODUCER_NONBLOCKING_LATEST_PHASE_CALLS,
    PROM_METRIC_MQ_PRODUCER_NONBLOCKING_LATEST_PHASE_RPS, PROM_METRIC_TOKIO_ALIVE_TASKS, PROM_METRIC_TOKIO_BUSY_PERCENT,
...
fn take_shm_file_metric(samples: &[PromSample]) -> HashMap<(String, String, String), f64> {
    ...
    for s in samples {
        ...
        out.insert((node.clone(), shm_dir_abs.clone(), file_path_abs.clone()), v);
    }
    out
}
